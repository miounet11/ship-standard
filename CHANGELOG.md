...
- 新增 `KERNEL-7`：本产品不变量表已写下并有测试。废止 `KERNEL-3`（K1–K10 编号只留在例子里）。
- 新增 `PATH-7`：验收拓扑是真实用户路径，反模式环境不计入通过。废止 `PATH-2`（嵌套互控仍是例子，不是全产品句）。
- 新增 `LAUNCH-11`：第三条配置路径可用，且不依赖在文档中公布主机地址。`LAUNCH-5` 保留、不可豁免；`LAUNCH-11` 同样不可豁免。
- `quality-kernel`、`acceptance-path` 升为 stable。`compound` 仍为 draft。
