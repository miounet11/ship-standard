...
- [ ] `product/README.md`（或等价权威表）存在，每类问题只链一份
- [ ] `STATUS.md` 为生成物，version 与包配置一致
- [ ] 打开的 S1 写在风险册里，不写在第三份手写状态里
- [ ] 本轮新 BUGFIX 已挂不变量编号，或已进 `docs/archive/`
- [ ] 仓库内没有文件名/标题含「最新」的现行文档
- [ ] 身份文件（AGENTS / clavue）不另写进度百分比
